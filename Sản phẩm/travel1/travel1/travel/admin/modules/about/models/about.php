...
class AboutModelsAbout extends FSModels
{
	...
	function __construct()
	{
		...
		$this->table_category_name = FSTable_ad::_('fs_about_categories');
		$this->table_name = FSTable_ad::_('fs_about');
		...
		$cmonth = date('m');
		//$cday = date('d');
		$this->img_folder = 'images/about/' . $cyear . '/' . $cmonth;
		$this->check_alias = 0;
	...
	function save($row = array(), $use_mysql_real_escape_string = 1)
	{
		...
		// related about
		// $record_relate = FSInput::get('about_record_related',array(),'array');
		// $row['about_related'] ='';
		// if(count($record_relate)){
		// 	$record_relate = array_unique($record_relate);
		// 	$row['about_related'] = ','.implode(',', $record_relate).',';	
		// }
		...
		if ($id) {
			...
			$row['created_time'] = FSInput::get('created_time');
			$row['updated_time'] = $time;
			$row['end_time'] = $time;
			$row['start_time'] = $time;
		...
		$path = PATH_BASE . 'images' . DS . 'image_banner' . DS . $cyear . DS . $cmonth . DS . $cday . DS;
		require_once(PATH_BASE . 'libraries' . DS . 'fsfiles.php');
		$upload = new  FSFiles();
		$upload->create_folder($path);

		$file_upload = $_FILES["image_banner"]["name"];
		if ($file_upload) {
			$tmp = $_FILES['image_banner']['tmp_name'];
			move_uploaded_file($tmp, $path . $file_upload);
			$row['image_banner'] = 'images/image_banner/' . $cyear . '/' . $cmonth . '/' . $cday . '/' . $file_upload;
		}
		//end save image
	...
	function get_about_related($about_related)
	{
		if (!$about_related)
			return;
		$query   = " SELECT id, title,image 
    					FROM " . $this->table_name . "
    					WHERE id IN (0" . $about_related . "0) 
    					 ORDER BY POSITION(','+id+',' IN '0" . $about_related . "0')
    					";
		global $db;
		$sql = $db->query($query);
	...
	function ajax_get_about_related()
	{
		$about_id = FSInput::get('new_id', 0, 'int');
