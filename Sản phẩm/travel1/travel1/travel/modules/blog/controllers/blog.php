...
class blogControllersblog extends FSControllers
{
	var $module;
	var $view;
	function display()
	{
		...
		// call models
		$model = $this->model;
		// $blog_list = $this->model->get_blog_list();
		$data = $model->getNews();
		// echo '123';
		// die;
		// $blog_list = $this->model->get_blog_list();
		...
		$breadcrumbs = array();
		$breadcrumbs[] = array(0 => FSText::_('Tin tức'), 1 => FSRoute::_('index.php?module=blog&view=home&Itemid=2'));
		$breadcrumbs[] = array(0 => $category->name, 1 => FSRoute::_('index.php?module=blog&view=cat&id=' . $data->category_id . '&ccode=' . $data->category_alias));
