<?php
	class tourControllerstour extends Controllers
	{
		...
		function add()
		{
			...
			$categories = $model->get_categories_tree();
			$list_key = array();
			// data from fs_tour_categories
			$categories_home  = $model->get_categories_tree();
			$maxOrdering = $model->getMaxOrdering();
			$uploadConfig = base64_encode('add|'.session_id());
            $list_key = $model->get_records(' new_id = "'.$uploadConfig.'"','fs_tour_keyword');
...
//			$tags_categories = $model->get_tags_categories();
        ...
        function save_key(){
            ...
            $record = $model->get_record_by_id($new_id,'fs_tour','image,title,alias');
            if($record){
            ...
            if($id){
                ...
                $rs = $model->_update($row,'fs_tour_keyword',' id = '.$id ,1);
                $rs = $id;
            }else{
                $row['created_time'] = $time;
                $row['edited_time'] = $time;
                $rs = $model->_add($row,'fs_tour_keyword',1);
            }
            //print_r($rs);
            $html = '';    
            if($rs){
                $data = $model->get_record_by_id($rs,'fs_tour_keyword');
                if($data){
        ...
        function delete_key(){
            $model  = $this -> model;
            $id = FSInput::get('id',0,'int');
            $rs = $model->_remove(' id = '.$id,'fs_tour_keyword');
            if($rs){
                echo 1;
            }else{
                echo 2;
