...
class tourModelstour extends FSModels
{
	function __construct()
	{
		...
		$this->table_name  = $fstable->_('fs_tour', 1);
		$this->table_category  = $fstable->_('fs_tour_categories', 1);
		$this->table_comment = $fstable->_('fs_tour_comments', 1);
	}
	//		function setQuery()
	//		{
	...
	function  get_relate_by_tags($tags, $exclude = '', $category_id)
	{
		...
		$query = " SELECT id,title,alias, category_id , image, category_alias,summary
						FROM " . $fs_table->getTable('fs_tour') . "
							" . $where . "
						ORDER BY  id DESC, ordering DESC
						LIMIT 0,$limit
						";
