<?php
class MemberModelsGame extends FSModels
{
	var $limit;
	var $prefix;
	function __construct()
	{
		$this->limit = 10;
		$this->view = 'game';
		$this->table_name = FSTable_ad::_('fs_user_games');
		parent::__construct();
	}
	function get_Game()
	{
		$fs_table = FSFactory::getClass('fstable');
        $query = " SELECT *  FROM " . $fs_table->getTable('fs_user_games', 1);
        global $db;
        // print_r($query);die;
        $sql = $db->query($query);
	...
	function setQuery()
	{
		...
		if (isset($_SESSION[$this->prefix . 'keysearch'])) {
			if ($_SESSION[$this->prefix . 'keysearch']) {
				...
				$where .= "AND fullname  LIKE '%" . $keysearch . "%' OR phone LIKE '%" . $keysearch . "%'  OR ip LIKE '%" . $keysearch . "%'  OR phone_contact LIKE '%" . $keysearch . "%'  ";
			}
		}
