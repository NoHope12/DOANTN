<?php
	class PartnersControllersPartners extends Controllers
	{
		...
		function display()
		{
			...
			// $categories = $model->get_partners_categories_tree_by_permission();
			$str_cat_id = '';
...
//			$tags_categories = $model->get_tags_categories();
        ...
        function add_html(){
            ...
            $link = "javascript: created_indirect('index.php?module=partners&view=partners',8,'add_link_$id')";
            $html = '';
        ...
        function save_key(){
            ...
            $record = $model->get_record_by_id($new_id,'fs_partners','image,title,alias');
            if($record){
               $row['new_title'] = $record->title;
               $row['new_image'] = $record->image;
               if(!$row['link_replace']){
                    $row['link_replace'] = 'index.php?module=partners&view=partners&code='.$record->alias.'&id='.$new_id.'&Itemid=4';
               }     
            }
            if($id){
            ...
            if($rs){
                ...
                if($data){
                    $link = "javascript: created_indirect('index.php?module=partners&view=partners',8,'link_$data->id')";
                    $html = '';
        ...
        function add_param(){
            ...
            include 'modules/'.$this->module.'/views/partners/add_param.php';
		}
		...
		function view_comment($new_id){
			$link = 'index.php?module=partners&view=comments&keysearch=&text_count=1&text0='.$new_id.'&filter_count=1&filter0=0';
			return '<a href="'.$link.'" target="_blink">Comment</a>'; 
		}
		
		function view_title($data){
			$link = FSRoute::_('index.php?module=partners&view=partners&id='.$data->id.'&code='.$data -> alias.'&ccode='.$data-> category_alias);
