<?php 
	class NewsModelsCategories extends FSModels
	{
		...
		function __construct()
		{
			...
			$this -> table_news = 'fs_abouts';
			$this -> table_name = 'fs_abouts_categories';
			$this -> check_alias = 1;
			parent::__construct();
		}
		...
		function save(){
			...
			if(@$parent_id)
			{
				$parent =  $this->get_record_by_id($parent_id,'fs_abouts_categories');
			...
			/*
		 * Update table fs_abouts_categories And table fs_abouts
		 */
		function update_parent($cid,$alias){
			$record =  $this->get_record_by_id($cid,'fs_abouts_categories');
			if($record -> parent_id){
				$parent =  $this->get_record_by_id($record -> parent_id,'fs_abouts_categories');
...
//			$row['root_alias'] = $root_alias;
			...
			if($id){
				...
				$this -> _update($row2,'fs_abouts',' category_id = '.$cid.' ');
				
				// update fs_abouts_categories : records have parent = this
		...
		function update_categories_children($parent_id,$root_id,$list_parents,$root_alias,$alias_wrapper){
			if(!$parent_id)
				return;
			$query = ' SELECT * FROM fs_abouts_categories 
...
//				$row3['root_alias'] = $root_alias;
				...
				if($this -> _update($row3,'fs_abouts_categories',' id = '.$item -> id.' ')){
					
					// update fs_abouts owner this category
...
//					$row2['category_root_alias'] = $row3['root_alias'];
					...
					$this -> _update($row2,'fs_abouts',' category_id = '.$item -> id.' ');
				}
