...
class destinationControllersDestination extends FSControllers
{
	var $module;
	var $view;
	function display()
	{
		// call models
		$model = $this->model;
		// $destination_list = $this->model->get_destination_list();
		$data = $model->getNews();
		// $destination_list = $this->model->get_destination_list();
		...
		$breadcrumbs = array();
		$breadcrumbs[] = array(0 => FSText::_('Tin tức'), 1 => FSRoute::_('index.php?module=destination&view=home&Itemid=2'));
		$breadcrumbs[] = array(0 => $category->name, 1 => FSRoute::_('index.php?module=destination&view=cat&id=' . $data->category_id . '&ccode=' . $data->category_alias));
